...
namespace VirtuSphere
{
    public partial class vmeditForm : Form
    {
        ...
        private async void LoadVMToFormFields(VM vm)
        {
            ...
            // Überprüfen Sie, ob die Datumsfelder korrekt sind und konvertieren Sie sie in einen String
            txtd_created_at.Text = selectedVM.created_at.ToString();
            txtd_updated_at.Text = selectedVM.updated_at.ToString();
            // Für Status könnte eine Konvertierung oder ein Mapping erforderlich sein
            txtd_status.Text = selectedVM.vm_status; // Angenommen, vm_status ist ein int oder enum
            ...
            {
                // Fügen Sie jedes Interface zur ListBox hinzu
                // Sie können hier entscheiden, welche Informationen angezeigt werden sollen,
                // z.B. nur die IP-Adresse oder eine Kombination von Eigenschaften
                //listBoxInterfaces.Items.Add($"{intf.ip}");
        ...
        private void MarkSelectedPackagesInListBox(List<Package> selectedPackages)
        {
            ...
            for (int i = 0; i < listBoxPackages2.Items.Count; i++)
            {
                ...
                Console.WriteLine("2MarkSelectedPackagesInListBox is: " + listBoxPackages2.Items[i]);
        ...
        {
            AddInterfaceToListBox(listBoxInterfaces, txtIP.Text, txtSub.Text, txtGateway.Text, txtDNS1.Text, txtDNS2.Text, ComboVLAN.Text, comboMode.Text);
    ...
    string txtdns1, string txtdns2, string txtvlan, string txtmode)
        {
            // Erstellen des Interface-Objekts
            Interface newInterface = new Interface
            {
